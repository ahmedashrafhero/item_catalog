...
app = Flask(__name__)
# load client_id  from client serects file
...
# create random state to pervent anti-forgery state token.
...
def gconnect():
    ...
    output += ''' " style = "width: 150px; height: px;border-radius:
...
def showRestaurants():
    ...
    if 'username' not in login_session:
        ...
        return render_template('restaurants.html',
                               restaurants=restaurants, user_id=user_id)
...
def newRestaurant():
    ...
    if request.method == 'POST':
        ...
        session.add(newRest)
        session.commit()
        flash("New Restaurant Created %s" % newRest.name)
...
def editRestaurant(restaurant_id):
    ...
    if editRest.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are \
                not authorized to edit this restaurant.\
    ...
    if request.method == 'POST':
            ...
            session.commit()
            flash("Restaurant Successufuly Edited %s" % editRest.name)
            return redirect(url_for('showRestaurants'))
    else:
            return render_template('editRestaurant.html',
                                   restaurant_id=restaurant_id, Rest=editRest)


@app.route('/restaurant/<int:restaurant_id>/delete', methods=['GET', 'POST'])
def deleteRestaurant(restaurant_id):
    Restdelete = session.query(Restaurant).filter_by(id=restaurant_id).one()
    if 'username' not in login_session:
        return redirect('/login')
    if Restdelete.user_id != login_session['user_id']:
    ...
    if request.method == 'POST':
        session.delete(Restdelete)
        session.commit()
        flash("%s Successufuly Deleted" % Restdelete.name)
        return redirect(url_for('showRestaurants'))
    else:
        return render_template('deleteRestaurant.html',
                               restaurant_id=restaurant_id, Rest=Restdelete)
...
def newMenu(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        session.add(newItem)
        session.commit()
        flash("New Menu %s Created" % newItem.name)
